...
grace가 봐주는 것:
  '원천이 더 최신'이 정상인 유일한 구간은 발표 직후~다음 배치 전이다. 배치는
  하루 한 번 돌고 이 검사는 그 뒤에 도니 그 창은 사실상 없다 — 그때도
  뒤처져 있으면 그날 배치가 못 받은 것이다. (감시의 실제 시작 시각은 GitHub 큐
  지연 탓에 예약보다 몇 시간씩 늦을 수 있다. 늦는 쪽은 안전하지만, 목요일
  12:00~17:00 KST 사이로 당겨지면 주간 오탐이 나므로 GRACE_WEEKLY를 10으로
  되돌려야 한다 — watchdog.yml의 schedule 주석에 실측 확인법이 있다.) 그래서 grace는 대부분 형식적이고(월간·기본통계는
...
# ⚠️ 여기 16곳은 **실제 행정구역만**이다. 저장분에는 집계 행이 섞여 있어
# (STATS 21곳 = 16시도 + 전국·수도권·지방·기타광역시·기타지방,
#  ADV.sido = 시도 + 전국·수도권·지방) 전부 더하면 전국의 세 배가 나온다.
# 개수가 19/21/16으로 달라 보이는 건 집계를 몇 개 안고 있느냐의 차이일 뿐이다.
# ⚠️ 이름은 SIDO17이지만 2026-09-10 광주·전남 통합으로 실제 길이는 16이다.
#    sido_zones.ORDER 에서 파생하므로 값은 늘 맞는다 — 이름만 옛것이다.
...
def check_sido_sum(stats):
    """시도 합(null→0) == 저장된 '전국' 행. **전 구간**을 본다.
...
def check_quiz_review(today, src=None):
    ...
    사람에게는 배치 기록의 ℹ️ 줄(batch_notes.quiz_review_lines — 월요일 확인 메일에 실린다)로 간다.
