...
def draft_weekly(adv, sts):
    ...
    ORD = ('첫째', '둘째', '셋째', '넷째', '다섯째')
    title = '[한국부동산원] 부동산 주간시장동향(%d월 %s 주) | 서울 %s 전국 %s' % (
    ...
    # '주간시장동향'을 태그로 받는다(2026-09-12) — 제목에서 빠진 '아파트 시세'는
    # 앞 두 태그가 지키고, 제목이 가져간 '주간시장동향'은 태그에도 있어야 검색
    # 의도 둘을 다 덮는다. '부동산데이터'는 검색 의도가 없는 말이라 뺐다.
    tags = ['주간아파트시세', '아파트시세', '주간시장동향', '집값전망', '아공맵']
    return dict(title=title, body='\n'.join(body), tags=tags, kw='부동산 주간시장동향',
...
#
# 목적지를 섞은 이유: 같은 곳으로만 보내면 이미 가 본 사람에게 두 번째부터
# 무의미하다. 지역 리포트는 3회에 한 번씩만 돌아온다.
ZONE_CTA = (
    ('이 숫자를 어떻게 냈는지 — 적정물량 기준선부터 분기별 물량까지 열어 뒀습니다.',
     '/zone/%s/', '%s 공급 리포트', 'zone_deep'),
    ('전세가율이 지금 어디까지 왔는지는 따로 정리해 뒀습니다.',
     '/jeonse-ratio/', '시도별 전세가율 현황', 'jeonse'),
    ('내 동네가 어느 칸인지는 여기서 바로 보입니다.',
     '/', '전국 공급 지도', 'home'),
    ('집값이 왜 도는지, 국가통계로 검증한 기록입니다.',
     '/cycle/', '아파트 사이클 리포트', 'cycle'),
    ('%s의 분기별 물량은 리포트에 그대로 있습니다.',
     '/zone/%s/', '%s 공급 리포트', 'zone_deep'),
    # ⚠️ 연도를 박지 않는다. '2026·2027년'으로 적어 두었는데, /moveins/가 보여주는
    # 구간은 매년 옮겨가므로 2028년부터는 글과 페이지가 다른 해를 말하게 된다
    # (2026-09-02 리뷰). 숫자 없이도 문장이 성립한다 — 신선도가 글에 의존하지 않게 둔다.
    ('앞으로 어디로 몇 세대가 들어오는지 미리 봐 두면 도움이 됩니다.',
     '/moveins/', '시도별 입주물량', 'moveins'),
    ('이번 주 시세는 지도로 보는 게 빠릅니다.',
     '/weekly/', '주간 시세 지도', 'weekly'),
    ('내 부동산 감각이 몇 점인지 5분이면 나옵니다.',
...
def cta(nm, seq):
    """글 끝 유도문 한 줄. 회차마다 문장과 목적지가 바뀐다."""
    txt, path, label, camp = ZONE_CTA[(seq - 1) % len(ZONE_CTA)]
    if '%s' in path:
        path = path % quote(nm)
    return ('<p>%s<br>👉 <a href="%s%s?utm_source=naver_blog&amp;'
            'utm_medium=social&amp;utm_campaign=%s">%s</a></p>'
...
def draft_zone(adv, sts, r, seq, total):
    ...
    if r.get('unsold'):
        if r.get('uwarn'):
            ...
            body.append('<p>미분양은 <b>%s호</b>입니다. 다 짓고도 팔리지 않은 재고라, '
    ...
    for i, (head, d) in enumerate(pairs, 1):
        ...
        if i == 1:
            ...
            S.append('<p class="note">✍ 해석을 쓴 뒤 <code>|</code> 뒤를 <b>결론 한 줄</b>로 '
                     '바꾸세요. 예: <code>[한국부동산원] 부동산 주간시장동향(9월 둘째 주) | '
