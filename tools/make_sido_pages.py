...
AGG_NOTE = {
    ...
    '지방': '수도권을 뺀 %d개 시도를 합친 값입니다.' % len(
        [z for z in SZ.ORDER if z not in SZ.AGG and SZ.REGION[z] == '지방']),
}

# 산식이 그 지역과 구조적으로 안 맞는 곳의 공시. est(추정치) 안내문과는 다르다 —
# est는 "값이 좀 부정확할 수 있다"는 방법론 얘기라 싣지 않기로 했지만(2026-08-08
# 사용자), 이건 "이 지역에선 지표 자체를 다르게 읽어야 한다"는 판정 정보다.
#
# 세종 근거(2026-08-08 역검증, 저점 3개 전수):
#   과거 가격 저점 직전 3년 공급이 적정의 5.7배(2015)·5.9배(2019)·2.2배(2023).
#   당시 세대수로 잣대를 다시 재도 10.7배·6.8배·2.1배 — 더 나빠진다. 도시가
#   공급으로 만들어져 세대가 공급을 따라 들어온 곳이라(2015년 8.2만 → 현재
#   16.6만 세대) '세대가 수요를 정한다'는 전제의 인과가 반대다.
...
def split_block(sp):
    """리포트 머리의 세 줄 블록(2026-09-13 대표 결정 A안·인허가 참고·추정 표기).
    ...
    if not sp:
        return ''
    out = ['<div class="zsplit">']
    for lab, txt in sp['rows']:
        if txt:
            out.append('<p class="zs-row"><b>%s</b><span>%s</span></p>' % (esc(lab), esc(txt)))
    if sp.get('ref'):
        ...
        out.append('<p class="zs-row zs-ref%s"><b><em class="zchip">참고</em>%s</b><span>%s</span></p>'
                   % (' zs-thin' if thin else '', esc(lab), body))
        out.append('<p class="zs-note">%s</p>' % esc(sp['ref_note']))
    if sp.get('est_note'):
        out.append('<p class="zs-est"><em class="zchip">추정</em>%s</p>' % esc(sp['est_note']))
    out.append('</div>')
    return ''.join(out)
...
if(localStorage.getItem('ga_off'))window['ga-disable-%(ga)s']=true;
}catch(e){}
</script>
...
def build_page(z, calc, stats, pq, others, weekly=None):
    ...
    # 숨김 결정을 뒤집었다(경기처럼 추정 여부가 해석을 좌우하는 경우가 생겼다). 카드에는
    # 결론만('추정' + 한 줄) 두고 방법론은 '어떻게 계산했나'가 맡는다. 목록은 SZ.EST에서
    # 파생된 row['est']를 쓴다 — 여기에 지역 이름을 적지 않는다.
    if z in MODEL_LIMIT_NOTE:
    ...
    h.append('<section><div class="wrap"><h2>어떻게 계산했나</h2>'
             ...
             # 넣었던 근거는 "결측은 –로 쓰면서 0은 맨숫자라 헷갈린다"였는데,
