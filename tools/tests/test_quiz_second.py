...
let same=true, keep=true, oxOrder=true, oxn=0; const first={}, total={}, pos={};
...
for(let s=1;s<=300;s++){
  for(const k of sets){
    ...
    a.forEach(it=>{
      ...
      const ox=it.opts.length===2&&it.opts.every(x=>x==='O'||x==='X');
      if(ox){ if(it.opts[0]!=='O'||it.opts[1]!=='X') oxOrder=false; oxn++; return; }
...
OUT={same,keep,first,total,sel,qn,varied,oxOrder,oxn};
"""
...
def test_options_are_shuffled_by_seed_without_changing_the_questions():
    ...
    O/X 문항은 섞지 않고 늘 O·X 순이다(2026-09-18 오딧 5번) — isOX 분기를 지우면 oxOrder 가 거짓이 된다.
    ...
    assert o['oxn'] > 0 and o['oxOrder'], 'O/X 문항이 O·X 순이 아니다(%d문항 검사) — "X | O" 로 나간다' % o['oxn']
