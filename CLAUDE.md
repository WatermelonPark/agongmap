# 아공맵 (aptweather) 작업 지침

아파트 공급 지도. 국가 통계(KOSIS·부동산원 R-ONE·한국은행 ECOS·공공데이터포털)를 그대로 가져와
시도·시군구 단위 공급 판정과 시세를 보여주는 정적 사이트다. 운영 배포는 **GitHub Pages**(`main`)이며 정식 도메인은
`CNAME`에 있는 것이다. PR에 붙는 Netlify(`aptweather`)는 병합 전 확인용 미리보기일 뿐 운영 사이트가 아니다. **이 저장소는 공개 저장소다.** 아래 "공개 저장소 규칙"을 먼저 읽는다.

## 공개 저장소 규칙 (하드 룰)

- 개인 인명을 적지 않는다. 타겟 사용자 집단은 "부동산 스터디 커뮤니티"처럼 집단명으로만 쓴다.
- 저장소 밖 로컬 경로(원고 폴더, 강의 아카이브, 키 파일 경로 등)를 문서·코드·커밋 메시지에 적지 않는다.
- 공인 IP, API 키, 개인 이메일을 적지 않는다. 키는 환경 변수로만 읽는다(`KOSIS_API_KEY`, `RONE_API_KEY`,
  `ECOS_API_KEY`, `DATA_GO_KR_KEY`). 서비스 대표 주소 `agongmap@gmail.com`은 개인 이메일이 아니라 공개 연락처이므로
  소개·개인정보·푸터에 적어도 된다(2026-09-18 대표 결정).
- 특정 전문가·강의를 출처로 인용하지 않는다. 출처는 기관명(국토교통부, 한국부동산원, 기획재정부 등)만 쓴다.
  개념은 배우더라도 **우리 공개 데이터로 검증해 우리 근거로** 만든다.

## 구조

| 위치 | 내용 |
|---|---|
| `index.html`, `app.css`, `data.js`, `data-*.json` | 홈과 데이터 페이로드. 데이터는 `data.js`의 `ADV_DATA` 블록을 배치가 갈아끼우고 `tools/split_data.py`가 `data-*.json`으로 쪼갠다 |
| `home-app.js` | 홈 본문 스크립트(2026-09-16 `index.html` 인라인에서 분리). 홈 코드는 여기서 고치고, 도구·시험은 이 파일을 직접 열지 말고 `tools/home_src.py`의 `home_source()`로 읽는다 |
| `zone/`(시도별 리포트), `weekly/`, `monthly/`, `moveins/`, `jeonse-ratio/` | 배치가 매 회차 굽는 생성 페이지. 손으로 고치지 말고 생성기를 고친다 |
| `cycle/` | 서술은 손으로 쓴 문서이고, 차트 데이터 배열만 `refresh_cycle_data.py`가 매 회차 갈아끼운다 |
| `share/` | 공유 이미지. `weekly-map.png`만 배치가 굽고, 지역·퀴즈 카드(`make_zone_cards.py` 등)는 지역 구성이나 문항이 바뀔 때 사람이 돌린다 |
| 퀴즈 3종(`burini-test/`, `investor-test/`, `redev-test/`) | 랜딩은 손으로 쓴 파일, 점수별 하위 페이지(`/0/`~)는 `make_quiz_share_pages.py`가 만든다 |
| `about/`, `faq/`, `privacy/` | 운영 주체·설명·개인정보 페이지 |
| `tools/` | 배치·생성기·발행 도구 (아래 표) |
| `tools/tests/` | pytest. 배치가 커밋 직전에 게이트로 돌린다 |
| `tools/data/` | 시딩 데이터·캐시·스탬프. `tools/cache/`와 벌크 원본은 gitignore |
| `docs/` | 세션 간 요청서·결정 기록·백로그. 날짜 접두 파일명. 목록은 `docs/README.md`, 종결된 요청서는 `docs/archive/`로 옮긴다 |
| `.github/workflows/` | 클라우드 배치·감시·발행 알림 (아래 "자동화") |
| `drafts/`, `logs/` | **로컬 전용(gitignore)**. 블로그 초안과 배치 로그. 클라우드 세션에서는 보이지 않는다 |

### 주요 도구

| 도구 | 역할 |
|---|---|
| `update_adv_data.py --update` | 원천 API에서 데이터를 받아 `data.js`를 갱신. 키가 필요하다(`--dry-run`은 없어졌다. 키 없이는 pytest로 검증한다) |
| `split_data.py` | `data.js` → `data-core.js`, `data-*.json` 분리 |
| `make_sido_pages.py`, `make_indicator_pages.py`, `make_monthly_page.py`, `make_weekly_page.py` | 시도·지표·월간·주간 페이지 생성. `refresh_cycle_data.py`와 함께 배치가 돌리는 생성기 다섯 개다 |
| `sido_zones.py` | 판정 단위(시도) 목록·등급·정렬의 **정본**. 순위는 반드시 `zone_order()`를 쓴다(등급군 → 절대량). 절대 세대수로 순위를 매기면 판정과 모순이 난다. 시도 **목록을 보여줄 때**는 순위가 아니라 `DISPLAY_ORDER`(관심 지역 고정 순서)를 쓴다. `/monthly/`처럼 정부 표와 대조하는 화면은 `ORDER`(발표 원천 순서)를 유지한다 |
| `check_freshness.py` | 데이터 신선도·정합성 검사. 감시 워크플로가 부른다 |
| `refresh_cycle_data.py` | `/cycle/` 사이클 리포트 데이터 |
| `make_zone_cards.py`, `make_og_cards.py`, `make_weekly_share.py` | 공유용 이미지·OG 카드 |
| `make_quiz_share_pages.py` | 퀴즈 점수별 정적 공유 페이지. 퀴즈 세트나 공유 이미지를 바꿨을 때 사람이 돌린다 |
| `make_naver_post.py`, `make_theory_post.py <편번호>` | 네이버 블로그 초안 생성 → `drafts/`. 배치가 부르지 않고 사람이 발행 직전에 돌린다 |
| `naver_serp.py` | 네이버 검색 API로 색인·순위 확인(`--index`, `--track`) |
| `run_weekly_update.bat` | 로컬 배치 러너(Windows 작업 스케줄러). 실패 시 즉시 중단하고 종료 코드로 사유를 남긴다 |

### 실행

```bash
python -m pytest tools/tests -q          # 전체 테스트
python tools/make_sido_pages.py
```

Python 3.12. 별도 requirements 파일은 없고 표준 라이브러리 위주이며, 이미지 도구는 `pillow`를 쓴다.
배치 게이트는 `pytest`·`pillow`만 깐다 — 시험과 배치 도구는 그 밖의 서드파티(yaml 등)를 모듈 맨 위에서 가져오지 않는다
(`test_test_deps_are_installed`). 개발 컨테이너엔 시스템 yaml 이 있어 로컬 pytest 로는 안 드러나므로, CI 와 같은 조건은
`python3 -m venv` 에 `pytest pillow` 만 깐 환경에서 확인한다(2026-09-24~26 게이트 정지 사례). 클라우드 세션은 훅이
그 환경을 만들어 준다(아래 "자동화"). 생성기·감시·fetch 잡은 `pip` 앞이나 설치 없이 돌므로 표준 라이브러리만 쓴다.
Windows 콘솔에서 한글이 깨지면 `PYTHONUTF8=1`을 준다.
로컬 미리보기는 `.claude/launch.json`의 `agongmap`(정적 서버, 8321 포트)을 쓴다.

## 자동화

- **데이터 갱신**: 배포 경로는 **클라우드 배치(`update-cloud.yml`) 하나**다. KOSIS가 해외 IP를 간헐 차단하므로
  러너 3개를 띄워 IP 프리플라이트를 통과한 것의 산출물만 쓰고, 0/3이면 35분 뒤 자동 재시도한다(`update-cloud-retry.yml`). 생성기 다섯
  개를 돌린 **뒤, 커밋 직전에 pytest를 돌려 실패하면 데이터를 커밋하지 않는다**(CI에서는 시험 건너뜀도 실패다).
  테스트가 깨져 있으면 배치가 며칠째 멈춘 것으로 나타난다. 로컬 러너(`run_weekly_update.bat`)는 2026-07-23
  이후 휴면 상태다(작업 스케줄러 항목이 꺼져 있음). 2026-09-23 대표 결정으로 **휴면 확정**이다(백로그 22). bat 은 비상용으로만 남긴다. 다시 켤 일이 생기면 로컬이
  클라우드보다 먼저 돌아 같은 데이터를 커밋하게 되므로 두 배치의 순서·대상이 같아야 한다(`test_batch_parity`).
- **시험**: `ci-tests.yml`이 PR·`main` 푸시·매일 예약(22:17 KST)에서 배치 게이트와 같은 조건(3.12, `pytest`·`pillow`만,
  depth 200, 생성기 먼저)으로 pytest 를 돌리고, 설치 없이 도는 진입점을 따로 돌려 본다. 둘이 같은지는
  `test_ci_workflow_mirrors_gate`가 본다. 봇 데이터 커밋은 푸시로 이 워크플로를 부르지 못해 예약 실행을 둔다 —
  배치 게이트가 경고로만 넘기는 '다음 분기 전진 시험'(`test_gate_survives_next_period`)이 여기서 실패로 돈다.
- **감시**: `watchdog.yml`이 매일 `check_freshness.py`로 라이브 데이터를 원천과 대조한다. 워크플로 자체가
  실패하면 GitHub이 실패 알림 메일을 보낸다. 배치 안의 알림 코드는 배치가 안 뜨면 실행되지 않으므로 감시자를 밖에 둔 것이다.
- **발행 알림**: `write-reminder.yml`이 화(지역·사이클 격주)·금(주간 시세) 아침에 블로그 발행 이슈를 연다.
  `publish-check.yml`이 발행을 확인하고 이슈를 닫는다.
- 클라우드 세션에는 API 키가 없다. 데이터 갱신(`update_adv_data.py`)을 돌리지 말고 pytest로 검증한다.
  클라우드 세션은 컨테이너가 매번 새로 뜨므로 `.claude/hooks/session-start.sh`(SessionStart 훅)가 시작 때
  `python3.12` 격리 가상환경에 `pytest`·`pillow`만 깔아 PATH 앞에 둔다(배치 게이트와 같은 판·같은 패키지). 3.12 가
  없는 컨테이너면 시스템 파이썬에 깔고 경고를 남기니, 그때는 PR 의 `ci-tests.yml` 결과로 한 번 더 확인한다.
  건너뜀까지 실패로 보려면 `GITHUB_ACTIONS=1 python3 -m pytest tools/tests -q`.
- 로컬에서만 할 수 있는 일(블로그 초안·발행, `drafts/`·`logs/`, 네이버 검색 API, 로컬 러너)은
  `docs/2026-09-23-로컬-전용-작업.md`에 모아 로컬 세션이 관리한다.

## 데이터 원칙

- **숫자는 원천에서 그대로 가져온다.** 원천이 합쳐서 발표한 값을 비율로 쪼개 채우지 않는다. 검증 불가능한
  수가 되기 때문이다. 원천 분류가 바뀌면 판정 단위 자체를 바꾸거나 그 달을 보류한다.
- 사이트와 블로그가 다른 숫자·다른 산식을 말하면 안 된다. 블로그 도구는 배치가 구운 값을 읽고 재계산하지 않는다.
- **사람이 센 수와 손으로 적은 목록을 코드에 박지 않는다.** 지역 수·시군구 수·검증 대상 곳 수
  같은 값은 모델이나 데이터에서 세어 쓴다(`len(SGG_QNAME)`, `CYCLE_SYNC_N`처럼). 박아 두면 모델이
  바뀌어도 따라오지 않고 **아무것도 빨개지지 않는다.** 2026-09-10 광주·전남 통합 한 번이 아홉
  자리에서 서로 다른 모습으로 터졌다(문장 속 수, 조건의 지역 목록, 완비 기준). 정본 상수를
  만들었으면 **같은 파일 안에서도 빠짐없이 그것을 쓴다** — 제목은 상수인데 본문은 하드코딩이면
  한 글 안에서 숫자가 갈린다.
- **같은 대상을 재는 코드가 둘 이상이면 같은 상수를 쓰고 그 일치를 시험으로 고정한다.** 서로 다른
  기준으로 재면 한쪽 방어선이 다른 쪽의 고장을 덮는다(2026-09-12 분양·미분양 수집 정지).
- **시험을 넣을 때 두 가지를 독스트링이나 커밋 메시지에 적는다.** ① 코드를 무엇으로 바꾸면 이 시험이
  빨개지는지(변이 1개 이상, 실제로 깨뜨려 확인), ② 픽스처가 어떤 실제 상태를 재현하는지. 초록인 채로
  결함이 배포되는 시험은 없는 것보다 나쁘다. 2026-09-16 리뷰 확정 15건 중 9건이 이 유형이었다(게이트가
  생성기 앞에서 돎, 평평한 픽스처, 검사기가 결함을 스스로 재현함).
- **게이트 시험은 날짜·데이터가 앞으로 가도 초록이어야 한다.** 실데이터의 달·분기·등급을 숫자로 박으면 새 분기가
  들어오는 날 게이트가 그날 데이터 커밋을 막는다(2026-09-26 데이터 감사: 20건 중 7건이 '데이터가 앞으로 가면 그날
  커밋이 막히는' 유형이었다 — 시험 5건, 생성기 중단 2건). 기대값은 데이터에서
  유도하고, 날짜가 지나서 생기는 일(검토 기한 등)은 게이트가 아니라 감시 경고·배치 기록 줄로 알린다. '오늘'은
  `tools/kst.py`, '전월'·'1년 전' 칸은 인덱스 차가 아니라 `sido_zones.month_back`으로 찾는다.
- 계산법 공개가 핵심 가치다. 산식을 바꾸면 사이트 설명문과 블로그 초안 생성기를 같이 고친다.
- 생성 페이지의 문구는 생성기(`make_*.py`)를 고친다. 출력물을 직접 고치면 다음 배치에서 되돌아간다.

## 세션 운영 (클라우드 1 + 로컬 1)

2026-09-23부터 세션을 둘로 줄였다. 개발·데이터·디자인·마케팅·콘텐츠·PM·리뷰 같은 역할은 세션이 아니라
백로그의 "담당" 칸으로 나눈다. 여기에 `stats-bot`(데이터 배치)이 매일 `main`에 커밋한다.

| 세션 | 하는 일 | 커밋 방식 |
|---|---|---|
| 클라우드 세션 | 코드·생성기·시험·문서 수정, pytest 검증 | 자기 컨테이너 클론에서 `claude/...` 브랜치 → PR → 병합 |
| 로컬 세션 | `docs/2026-09-23-로컬-전용-작업.md`의 일(블로그 초안·발행, 네이버 검색, 키가 필요한 조회) | 로컬 작업 트리에서 `main`에 직접 |

공통 규칙:

- 세션 간 연락은 `docs/` 요청서와 `docs/2026-09-09-유지보수-백로그.md`로 한다. 항목을 집을 때 세션명·시각을
  적고 `🔨 진행중` 커밋을 남기며, 끝내면 `✅ 완료`와 커밋 해시를 적는다.
- **완성한 작업은 바로 커밋한다.** 다른 세션은 내 검증 기록을 못 보므로 미커밋 상태는 "중간 상태"로 오해된다.
- 푸시 직전에 `git fetch`로 원격 상태를 확인한다. 로컬 세션은 작업을 시작할 때 `git pull`부터 한다.
- 커밋 메시지는 한국어 한 줄 요약(무엇을, 왜). 요청서 번호나 백로그 번호가 있으면 함께 적는다.
- `git add -A`, `git stash`, `--autostash`를 쓰지 않는다. 남의 미커밋 작업을 쓸어 담거나 숨긴다.

클라우드 세션(PR 브랜치):

- **생성 산출물을 PR에 싣지 않는다.** `data.js`, `data-*.json`, `zone/`·`weekly/`·`monthly/`·`cycle/` 등 생성
  페이지, `share/` 이미지는 `stats-bot`이 매일 `main`에 다시 쓰므로, PR 브랜치에서 고치면 병합 때 충돌하고 다음
  배치에서 덮인다. 생성기(`make_*.py`)와 시험만 고치고 산출물은 배치가 굽게 둔다. 생성기를 돌려 확인만 했다면
  산출물 변경은 커밋 전에 되돌린다.
- 병합 전 CI(`ci-tests.yml` 시험·Netlify 미리보기)가 초록인지 본다. 운영 반영은 `main` 병합 뒤 GitHub Pages 배포로 이뤄진다. 병합이 늦어져 `main`과 충돌하면 `main`을 병합해 푼다.

로컬 세션(공유 작업 트리):

- **작업 트리와 인덱스를 로컬의 모든 작업이 같이 쓴다.** 스테이징해 두고 다른 일을 하면 남의 커밋에 실린다
  (2026-09-16 병합 커밋 930b4534 사례).
  - `git add`와 `git commit`은 한 명령으로 붙여 실행한다.
  - 커밋 전 `git status` 전체(잘라 보지 않는다)에서 충돌 파일(U)·`MERGE_HEAD`·남의 미커밋 변경을 본다. 있으면
    그 트리에서 커밋하지 말고 `git worktree add --detach <임시 경로> origin/main`에서 커밋·푸시한 뒤 워크트리를 지운다.

## 블로그 글 (마케팅·콘텐츠 담당)

주력 유통 채널은 네이버 블로그다. 주간 시세(금), 지역 공급(격주 화), 부동산 사이클 이론(격주 화) 세 카테고리.
운영은 3단이다: 생성기가 표·순위·링크를 채우고, Claude가 그 주 데이터의 해석 문단을 쓰고, 사용자가 검토·발행한다.

- **우리는 공급론자다.** 방어적으로 쓰지 않는다. 부정형으로 문단을 열지 않고, "다만/대신"으로 강한 사실을
  감싸지 않는다. 한계는 근거 옆에 짧게 둔다.
- **사실 나열로 끝내지 않는다.** 숫자 다음에는 "그래서 어떻게 되는가"를 쓴다. 두 갈래인데 결론이 같은 논증을
  우선 찾는다.
- **구조**: 결론 → 그 주제의 내용 → 전망·해석 → (링크) → 산식·방법론 → 면책. 방법론은 맨 뒤에 짧게.
  도구 설명서가 아니라 그 주제의 리포트로 쓴다. 소제목에 브랜드를 붙이지 않는다.
- **회차 간 반복 금지.** 매주 같은 설명(산식, 서비스 소개, 사이클 고리)을 다시 쓰지 않는다. 숫자가 지역마다
  다른 문장만 리포트 본체다.
- **지키는 선**: 투자자문 아님 면책, 시점 한계("방향은 되고 시점·폭은 안 된다"), 데이터와 견해를 가르는
  "제 판단으로는", 사이트 정본과 다른 말 금지.
- **제목**: 주간 시세는 `|` 뒤에 결론절, 지역·사이클은 쉼표. 검색결과에 30자쯤만 보이므로 앞에 검색어, 뒤에 결론.
- 사이트 문장을 그대로 옮기면 유사문서로 밀린다. 블로그는 "왜 그런가"를 사람 말로 풀고 숫자 근거는 사이트로 링크한다.
- 결론은 절대 공급 부족으로 다시 묶는다. 세제·금리 같은 외부 요인을 다룬 글일수록 그렇다.

## 응답 언어

사용자에게는 한국어로 답한다. 문장은 조사·어미를 생략하지 않은 완결된 형태로 쓰고, 명사구나 연결어미로
끝내지 않는다. 비유적 어휘 대신 일반 어휘를 쓰고, 엠대시(—)는 쓰지 않는다. 코드·주석·커밋 메시지는 프로젝트
관례를 따른다. (사용자 전역 지침 "fluent-korean"의 요약이다. 전문은 사용자 로컬 `~/.claude/CLAUDE.md`에 있다.)

**시각은 항상 한국 시간(KST)으로 말한다.** 사용자에게 시각·일정·예약을 알릴 때는 UTC 등 다른 시간대를 쓰지 않고
KST로 바꿔 적는다(예: "22:28", 필요하면 "(KST)"). GitHub Actions cron·API 응답처럼 원본이 UTC인 값도 사용자에게는
KST로 환산해 전한다. 워크플로 파일 안의 cron 식은 UTC 그대로 두되 주석에 KST를 함께 적는 기존 관례를 따른다(2026-09-26 대표 요청).
